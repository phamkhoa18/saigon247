...
const australianCities = [
  ...
  // Reset customerSelectedDate khi departureDates thay đổi
  useEffect(() => {
      ...
      <Card>
        <CardContent className="flex flex-col gap-6 py-6">
          ...
          <div>
              ...
              {formData.thumbnail && (
                <p className="mt-2 text-sm text-gray-600">Ảnh đã chọn: <a href={formData.thumbnail} target="_blank" rel="noreferrer" className="text-blue-600 underline">{formData.thumbnail}</a></p>
              )}
          </div>

          <div>
              <Label className='mb-2.5' htmlFor="featuredImage">Hình ảnh tour ( Danh sách hình ảnh )</Label>
              <MultiImageUploader onUploadSuccess={handleMultiImageUpload}></MultiImageUploader>
          </div>
          <div>
            ...
            {formData.itinerary.map((item, idx) => (
              <div key={idx} className="mb-4 p-4 border rounded">
                ...
                <TextEditor
                    value={item.content}  // thay vì formData.overview
                    onChange={(content) => {
                      const newItinerary = [...formData.itinerary];
